...
<#macro main run>
	<#if run.launched>
		...
		<#assign timeString><#if run.running>time:(from:'${run.startDate?iso_utc}',mode:relative,to:now)<#else>time:(from:'${run.startDate?iso_utc}',mode:absolute,to:'${run.endDate?iso_utc}')</#if></#assign>
		...
		<a href="${url}" target="_blank"><span class="glyphicon glyphicon-new-window" aria-hidden="true"></span> open in Kibana</a>
		<iframe 
			src="${url}&embed=true" 
			class="embeddedApp"
