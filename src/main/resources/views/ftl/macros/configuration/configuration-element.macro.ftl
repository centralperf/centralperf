			...
			<#else>
				<div class="col-md-9 input-group">
					<#if keyData.isReadOnly()>
						<input type="text" id="${keyData.keyName}" value="${keyData.keyValue}" class="form-control"
							   disabled/>
					<#else>
						<input type="text" id="${keyData.keyName}" value="${keyData.keyValue??}"
							   defaultvalue="${keyData.keyValue??}" class="form-control cpConfInput"/>
					</#if>

					<span id="revert-${keyData.keyName}" class="input-group-addon"
						  <#if !keyData.isFromDb()>style="display: none;"</#if>>
						<a href="${rc.contextPath}/configuration/revert/${keyData.keyName}/${group}"
						   title="Revert to properties value (delete from database)">
