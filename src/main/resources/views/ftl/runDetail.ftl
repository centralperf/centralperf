...
<#import 'macros/run/run-modal-output.macro.ftl' as run_modal_output>
...
<@layout.main title="Run detail" menu="runs">
    <div class="page-header page-title">
        ...
        <a href="#" id="runLabelEditable" data-name="label" data-type="text"
           data-url="${rc.contextPath}/run/${run.id?c}" data-title="Enter run name">${run.label}</a>
        <small><em>
                (<a
        ...
        <span class="pull-right" style="vertical-align:middle">
			<#if !run.running>
                <#if run.launched && !run.finished && run.scheduledDate?has_content>
                    <span class="badge">
                        <span class="glyphicon glyphicon-time" title="Scheduled" aria-hidden="true"></span>
                        Scheduled to start <@utils.prettyDate run.scheduledDate/>
                    </span>
                    <a data-toggle="modal" href="#run-modal-schedule" class="btn btn-success">
                        <span class="glyphicon glyphicon-time"></span><b> Modify schedule</b>
					</a>
                    <a href="${rc.contextPath}/project/${run.project.id?c}/run/${run.id?c}/stop" class="btn btn-danger">
						<span class="glyphicon glyphicon-off"></span><b>&nbsp; Cancel</b>
					</a>
				<#elseif run.launched && !run.finished && run.scheduleCronExpression?has_content>
                    <span class="badge">
                        <span class="glyphicon glyphicon-refresh" title="Scheduled" aria-hidden="true"></span>
                       ${cronExpressionReadable}. Next start <@utils.prettyDate nextStartDate/>
                    </span>
                    <a data-toggle="modal" href="#run-modal-schedule" class="btn btn-success">
                        <span class="glyphicon glyphicon-refresh"></span><b> Modify schedule</b>
					</a>
                    <a href="${rc.contextPath}/project/${run.project.id?c}/run/${run.id?c}/stop" class="btn btn-danger">
						<span class="glyphicon glyphicon-off"></span><b>&nbsp; Cancel</b>
					</a>
            ...
            <#if run.launched && run.lastStartDate??>
                <a data-toggle="modal" href="#run-modal-output" class="btn btn-info">
						<span class="glyphicon glyphicon-list-alt"></span><b> Logs</b>
				</a>
            </#if>
        </span>
    </div>
